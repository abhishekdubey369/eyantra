...
class LifeFormDetector:
    def __init__(self):
        ...
        self.target_organism = "alien_b"
        self.detected_organisms = []
        self.landing_completed = False
        self.swift_drone = SwiftDrone()  # Initialize Swift Drone here
    ...
    def detect_organisms(self, image):
        ...
        if num_leds >= 2 and num_leds <= 4:
            ...
            if organism_type == self.target_organism and not self.landing_completed:
                self.land_drone();self.swift_drone.disarm()
...
def fly_in_circular_motion(swift, radius, angular_speed, duration):
    ...
    # Stop the drone after the desired duration
    stop_cmd = Twist()
    swift.cmd_vel_pub.publish(stop_cmd);swift.disarm()
...
class SwiftDrone:
	...
	def update_setpoint(self, radius, angular_speed):
		...
		setpoint = [
    		radius * math.cos(angular_speed * time_since_start),
    		radius * math.sin(angular_speed * time_since_start),
...
if __name__ == '__main__':
    try:
        ...
        fly_in_circular_motion(swift_drone, radius=5.0, angular_speed=0.3, duration=120.0)
        LifeFormDetector()
        rospy.spin()
